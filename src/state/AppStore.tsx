...
import { analyseIntegrity, summariseFindings, type IntegritySummary } from "@/lib/integrity";
...
type Action =
  ...
  | { type: "findings"; findings: IntegrityFinding[] }
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case "findings":
      if (!state.document) return state;
      return { ...state, document: { ...state.document, findings: action.findings } };
    case "questions":
      return {
        ...state,
...
export function AppStoreProvider({ children }: { children: ReactNode }): ReactNode {
    ...
    async (fileName: string, bytes: Uint8Array) => {
      try {
        ...
        if (document.references.length > 0) {
          ...
          // Re-run the integrity analysis now that registry records are in hand:
          // author mismatches, identifier mismatches and fabricated references
          // can only be established with them.
          const verified: ReportDocument = {
            ...document,
            references: document.references.map((reference) => ({
              ...reference,
              authenticity: verdicts.get(reference.id) ?? reference.authenticity,
            })),
          };
          dispatch({ type: "findings", findings: analyseIntegrity(verified) });
        }
  ...
  const buildCheckpoint = useCallback(async () => {
    ...
    try {
      ...
      dispatch({
        type: "notice",
        notice: `${plan.questions.length} questions prepared: ${plan.fromFindings} targeted at specific integrity findings, the rest ${
          plan.usedModel ? "generated by the local model" : "from the built-in bank (no local model answered)"
        }.`,
      });
  ...
  const integrity = useMemo(
    () =>
      state.document
        ? summariseFindings(state.document.findings, state.document.references.length)
        : null,
    [state.document],
  );
